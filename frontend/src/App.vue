<template>
  <div id="app">
  <!-- Loader starts-->
    <div class="loader-wrapper" :class="{'loderhide' : !$store.state.menu.loading }">
...
<script>
  ...
  export default {
    ...
    /**
     * Try to authenticate with token in local storage
     * */
    async created() {
      this.$store.commit("menu/setLoading", true);

      const tokenCheck = this.$store.state.user.tokenCheck;
      await tokenCheck.init();



      if (this.$route.meta.requireAdmin) {
        if (!this.$store.state.user.user.organizations.length) {
          await this.$router.push({ name: 'home' });
        }
      } else if (this.$route.meta.requireSuperAdmin) {
        if (!this.$store.state.user.user.is_super_admin) {
          await this.$router.push({ name: 'home' });
        }
      }

      if (tokenCheck.isAuthenticated()) {
        const me = tokenCheck.getMe();
